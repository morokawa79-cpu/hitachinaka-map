# 水戸市マップ 元データのダウンロード
# 実行場所: C:\mito-build  例: powershell -File mito-download.ps1
$ua = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/124.0 Safari/537.36"

# 1) 大字（町丁・字等）境界 TopoJSON … NII Geoshape（総務省 令和2年国勢調査）
Invoke-WebRequest -UserAgent $ua -OutFile "r2ka08201.topojson" `
  -Uri "https://geoshape.ex.nii.ac.jp/ka/topojson/2020/08/r2ka08201.topojson"
...
# 4) 用途地域 A29（茨城県=08, 2019年）… 国土数値情報
...
Write-Host "その後: node mito-process.mjs / mito-assign-schools.mjs / mito-build-chome.mjs / mito-build-yochi.mjs / mito-build-poi.mjs"
