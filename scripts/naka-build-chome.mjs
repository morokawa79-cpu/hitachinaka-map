...
fs.writeFileSync(`${OUT}/naka-chome.js`, 'window.CHOME_GEO = ' + JSON.stringify(turf.featureCollection(kept)) + ';\n', 'utf8')
const kb = Math.round(fs.statSync(`${OUT}/naka-chome.js`).size/1024)
console.log(`naka-chome.js: ${kept.length}/${feats.length} 区域 (${kb} KB), 丁目を持つ大字数: ${Object.values(cnt).filter(c=>c>=2).length}`)
