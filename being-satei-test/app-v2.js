...
function render(result){
  ...
  output.innerHTML=`<p class="result-summary">${summary}</p>
    ...
    ${i.kind==='house'?`<div class="breakdown"><div><span>土地の査定額</span><strong>${man(result.landValue)}<small>万円</small></strong><p class="breakdown-units">${unitPrices(result.landValue/i.landArea)}<small>土地面積で換算</small></p></div><span class="plus">＋</span><div><span>建物の査定額</span><strong>${man(b.value)}<small>万円</small></strong><p class="breakdown-units">${unitPrices(b.value/i.buildingArea)}<small>延床面積で換算・減価後</small></p></div></div>`:''}
    <div class="calculation-note"><p><strong>土地</strong>　${result.basis==='comparables'?`${count}件の土地事例の単価${count===1?'':'中央値'}`:'公示地価の単価'} ${unitPrices(result.landUnit)}<br>㎡単価 × ${fmt.format(i.landArea)}㎡で計算</p>${i.kind==='house'?`<p><strong>建物</strong>　${esc(b.label)}・参考工事単価（減価前）<br>${unitPrices(b.unitCost)}<br>㎡単価 × ${fmt.format(i.buildingArea)}㎡ × 残価率 ${fmt.format(b.remainingRate*100)}％</p><p class="small">築年数で減価する前の参考額：${man(b.replacement)}万円。築${i.age}年・耐用年数${b.usefulLife}年で減価しています。</p><p class="small">建物は参考単価と築年数から概算。仕様・状態により変わります。</p>`:''}</div>
