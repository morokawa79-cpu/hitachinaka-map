// 水戸市 用途地域（国土数値情報 A29-19）→ data/mito-yochi.js
import fs from 'fs'
import * as turf from '@turf/turf'

const OUT = 'G:/マイドライブ/Claude/hitachinaka-map/data'

// A29 の水戸市 GeoJSON を探す
let SRC = null
const candidates = [
  'A29/A29-19_08/01-03_GeoJSON形式/A29-19_08201.geojson',
  'A29/A29-19_08201.geojson',
  'A29/A29-19_08/A29-19_08201.geojson',
]
...
if (!SRC) {
  ...
  const walk = (dir) => {
    ...
    for (const f of fs.readdirSync(dir)) {
      ...
      if (f === 'A29-19_08201.geojson') return p
    }
    return null
  }
  SRC = walk('A29')
}
if (!SRC) throw new Error('A29-19_08201.geojson が見つかりません。A29/ フォルダを確認してください。')
console.log('SRC:', SRC)
...
fs.writeFileSync(`${OUT}/mito-yochi.js`, 'window.YOCHI_GEO = ' + JSON.stringify(turf.featureCollection(feats)) + ';\n', 'utf8')
const kb = Math.round(fs.statSync(`${OUT}/mito-yochi.js`).size/1024)
const types = [...new Set(feats.map(f=>f.properties.name))]
console.log(`mito-yochi.js: ${feats.length} 区域 (${kb} KB)`)
